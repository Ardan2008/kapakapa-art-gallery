...
<head>
    ...
    <title>Kuthoadem Gallery | Add New Collection</title>
                                        ...
                                        <div class="flex items-center gap-6">
                                            <div class="w-24 h-24 flex-shrink-0">
                                                ...
                                                <div onclick="document.getElementById('profileInput').click()" class="group relative w-full h-full rounded-2xl overflow-hidden border border-white/5 cursor-pointer bg-neutral-800/50">
                                                    <img id="profileDisplay" src="https://api.dicebear.com/8.x/notionists/svg?seed=artist" class="w-full h-full object-cover opacity-50 group-hover:opacity-100 transition-all" />
                                            ...
                                            <div class="flex-1">
                                                ...
                                                <input type="text" name="artist[name]" placeholder="Full Name" class="w-full bg-neutral-800/30 border border-white/5 text-white rounded-xl px-5 py-3.5 focus:border-[#C9A74E]/50 outline-none transition-all text-sm font-bold" required>
                                            </div>
                                        </div>
                                        <div class="grid grid-cols-2 gap-4">
                                            <div>
                                                <label class="block text-[10px] text-gray-500 font-black uppercase tracking-widest mb-3">Birthplace</label>
                                                <input type="text" name="artist[birthplace]" placeholder="City, Country" class="w-full bg-neutral-800/30 border border-white/5 text-white rounded-xl px-5 py-3.5 focus:border-[#C9A74E]/50 outline-none transition-all text-sm font-bold" required>
                                            </div>
                                            <div>
                                                <label class="block text-[10px] text-gray-500 font-black uppercase tracking-widest mb-3">Career</label>
                                                <input type="text" name="artist[career]" placeholder="e.g. 2015 - Now" class="w-full bg-neutral-800/30 border border-white/5 text-white rounded-xl px-5 py-3.5 focus:border-[#C9A74E]/50 outline-none transition-all text-sm font-bold" required>
                                            </div>
                                        </div>
                                        <div>
                                            <label class="block text-[10px] text-gray-500 font-black uppercase tracking-widest mb-3">Biography</label>
                                            <textarea name="artist[desc]" placeholder="Brief story..." class="w-full bg-neutral-800/30 border border-white/5 text-white rounded-2xl px-5 py-4 focus:border-[#C9A74E]/50 outline-none transition-all text-sm font-bold h-24" required></textarea>
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div class="pt-10 border-t border-white/5 space-y-6">
                                    <div class="flex items-center justify-between">
                                        ...
                                        <div class="flex items-center gap-3">
                                            ...
                                            <input type="number" id="collection-qty" value="1" min="1" max="10" oninput="updateCollectionSlides(this.value)" class="w-16 bg-neutral-800 border border-[#C9A74E]/30 text-white rounded-lg px-3 py-2 text-xs font-bold focus:border-[#C9A74E] outline-none transition-all">
                                        </div>
                                    </div>
                            ...
                            <div class="space-y-6">
                                ...
                                <div class="grid grid-cols-3 gap-3">
                                    <div>
                                        ...
                                        <input type="number" step="0.01" name="artwork[${i}][width]" placeholder="W" class="w-full bg-neutral-800/50 border border-white/5 text-white rounded-xl px-4 py-3 focus:border-[#C9A74E]/50 outline-none transition-all text-xs font-bold text-center">
                                    </div>
                                    <div>
                                        <label class="block text-[8px] text-gray-600 font-black uppercase tracking-widest mb-2">Height</label>
                                        <input type="number" step="0.01" name="artwork[${i}][height]" placeholder="H" class="w-full bg-neutral-800/50 border border-white/5 text-white rounded-xl px-4 py-3 focus:border-[#C9A74E]/50 outline-none transition-all text-xs font-bold text-center">
                                    </div>
                                    <div>
                        ...
                        <div class="grid grid-cols-2 gap-8 pt-6 border-t border-white/5">
                            <div class="space-y-6">
                                ...
                                <div class="relative">
                                    ...
                                    <input type="number" name="artwork[${i}][salePrice]" placeholder="Sale Price" class="w-full bg-neutral-800/50 border border-white/5 text-white rounded-xl pl-8 pr-4 py-3 focus:border-[#C9A74E]/50 outline-none transition-all text-xs font-bold" required>
                                </div>
                            </div>
                            <div class="space-y-6">
                                ...
                                <div class="grid grid-cols-4 gap-3">
                                    <div class="col-span-3 grid grid-cols-3 gap-2">
                                        ${[0, 1, 2].map(j => `
                                            <div class="preview-box" id="mediaBox_${i}_${j}" onclick="handleMediaClick(${i}, ${j})">
                                                <input type="file" name="artwork[${i}][media][]" id="mediaInput_${i}_${j}" accept="image/*" class="hidden" onchange="previewMedia(this, ${i}, ${j})">
                                                <div id="mediaPlaceholder_${i}_${j}" class="absolute inset-0 flex flex-col items-center justify-center text-neutral-600 group">
                                    ...
                                    <div class="col-span-1">
                                        <input type="file" name="artwork[${i}][certificate]" id="certInput_${i}" accept="image/*,.pdf" class="hidden" required onchange="handleCertUpload(this, ${i})">
                                        <div onclick="document.getElementById('certInput_${i}').click()" class="preview-box border-red-500/20" id="certBox_${i}">
                                            <div id="certPlaceholder_${i}" class="absolute inset-0 flex flex-col items-center justify-center text-red-500/40">
